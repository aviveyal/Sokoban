...
public class LevelDisplaer{
	...
	BoxOntarget fakebox = new BoxOntarget(-1,-1);//in case there are no box on targets
	Display.getBoxOnTareget().add(fakebox);
	
		
	boolean printedBOT = false; // if printed a box - for not printing " " too
	...
	while(wallsnumber<Display.getWalls().size() && targetsnumber<Display.getTargets().size() 
			...
			for(int x=0;x<=maxrowsize;x++)
			{
				...
				{	
					...
					for(int i=0; i<Display.getBoxes().size();i++) // boxes can change  the order in the array !
					{
						if((Display.getBoxes().get(i).getX()==x )&&(Display.getBoxes().get(i).getY()==y ))
						{
							for(int s=0; s<Display.getBoxOnTareget().size();s++) // box arrived to the the target ! 
							{
								if((Display.getBoxOnTareget().get(s).getX()==x)&&(Display.getBoxOnTareget().get(s).getY()==y ))
								{
