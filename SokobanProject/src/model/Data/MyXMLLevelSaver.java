...
public class MyXMLLevelSaver implements LevelSaver {
	...
	public void savelevel(FileOutputStream outputStream, Level levelSaver) throws IOException {
		...
		XMLEncoder e = new XMLEncoder(new BufferedOutputStream(outputStream)); //using XML encoder
				...
				while(wallsnumber<wall.size() && targetsnumber<target.size() 
						&& boxesnumber<box.size() && sokonumber<soko.size()){
					...
					for(int x=0;x<=maxrowsize;x++)
					{
						...
						{	
							...
							for(int i=0; i<levelSaver.getBoxes().size();i++) // boxes can change  the order in the array !
							{
								if((levelSaver.getBoxes().get(i).getX()==x )&&(levelSaver.getBoxes().get(i).getY()==y ))
								{
									for(int s=0; s<levelSaver.getBoxOnTareget().size();s++) // box arrived to the the target ! 
									{
										if((levelSaver.getBoxOnTareget().get(s).getX()==x)&&(levelSaver.getBoxOnTareget().get(s).getY()==y ))
										{
											e.writeObject(levelSaver.getBoxOnTareget().get(s).boxesontargetsymbol());
											printedBOT= true;
											s=levelSaver.getBoxOnTareget().size();
											
											targetsnumber++; //printed V on a place with target 
