 ...
 * 
 *         the algorithm : 1. check if next step is empty -> move soko 2. check
 *         if there is box on target next if yes - 1. check if 2 steps next is
 *         empty - delete Box on target object , move soko move box 2. check 2
 *         steps next is another target -delete Box on target object,create new
 *         box on target, move soko move box if no - 3.check if two steps next
 *         is a target - move soko , move box , create new Box on target object
 *         4.check if two steps forward is empty - move soko , move box
 * 
 * 
 * 
 *         can be written shorter - add another function and for each way to
 *         move change how much to move -function(w,x-1,y),function(s,x+1,y)
 * 
 */
	...
	public void MovePlayer(Level playlevel, int direction) {
		...
		if ((char) direction == 'w')// move up
		{
			...
			if (Policy.CanMove(playlevel, 'w')) {
				...
				for (int i = 0; i < playlevel.getSokoCharas().size(); i++) {
					if ((playlevel.getSokoCharas().get(i).getX() == x)
							&& (playlevel.getSokoCharas().get(i).getY() == y))// change
																				// the
																				// specific
																				// soko
																				// position
				...
				for (int i = 0; i < playlevel.getBoxOnTareget().size(); i++) {
					if ((playlevel.getBoxOnTareget().get(i).getX() == x - 1)
							&& (playlevel.getBoxOnTareget().get(i).getY() == y)) {
				...
				// move box
				for (int i = 0; i < playlevel.getBoxes().size(); i++)// check
																		// all
																		// boxes
				{
				...
				for (int i = 0; i < playlevel.getBoxes().size(); i++)// check
																		...
																		// boxes
				{
					if ((playlevel.getBoxes().get(i).getX() == x - 1) && (playlevel.getBoxes().get(i).getY() == y))// change
																													// the
																													// specific
																													// box
																													// position
					{
						...
						playlevel.getBoxes().get(i).position(x - 2, y);
						playlevel.getBoxOnTareget().add((new BoxOntarget(x - 2, y)));// create
																						// new
																						// box
																						// on
																						// target
		...
		else if ((char) direction == 'a')// move left
		{
			if (Policy.CanMove(playlevel, 'a')) {
				...
				// remove current V , add new V next step
				for (int i = 0; i < playlevel.getBoxOnTareget().size(); i++) {
					if ((playlevel.getBoxOnTareget().get(i).getX() == x)
							&& (playlevel.getBoxOnTareget().get(i).getY() == y - 1)) {
						playlevel.getBoxOnTareget().remove(i);
				...
				for (int i = 0; i < playlevel.getBoxes().size(); i++)// check
																		...
																		// boxes
				{
					if ((playlevel.getBoxes().get(i).getX() == x) && (playlevel.getBoxes().get(i).getY() == y - 1))// change
																													// the
																													// specific
																													// box
																													// position
					{
						...
						playlevel.getBoxes().get(i).position(x, y - 2);
						playlevel.getBoxOnTareget().add((new BoxOntarget(x, y - 2)));// create
																						// new
																						// box
																						// on
																						// target
