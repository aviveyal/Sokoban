			...
			Plannable plannable = new Plannable() {
				...
				public Action getsatisfyingAction(Predicate top) {
					...
					if (top.getType().startsWith("boxAt")) {
						...
						// player position
						loop:
						for (Predicate player : kb.getPredicates()) {
							
							if (player.getType().startsWith("sokobanAt")) {
								int rowp = Integer.parseInt(player.getValue().substring(0, player.getValue().indexOf(',')));
								int colp = Integer.parseInt(player.getValue().substring(player.getValue().indexOf(',') + 1, player.getValue().length()));
						...
						for (Predicate p : kb.getPredicates()) {
							...
							if (p.getType().startsWith("tar")&& p.getID().substring(1).equals(top.getID().substring(1))) {
						...
						for (Predicate p : kb.getPredicates()) {
							...
							if (p.getType().startsWith("box")&& p.getID().equals(top.getID())) {
								...
								colb = Integer.parseInt(p.getValue().substring(p.getValue().indexOf(',') + 1, p.getValue().length()));
						...
						//	System.out.println(sol1);
							...
							if (sol2 != null ){
								...
								action.getPreconditions().add(new Predicate("targetAt", "t" +top.getID().substring(1), top.getValue()));// boxnumber=
								...
								int newcol = Integer.parseInt(top.getValue().substring(top.getValue().indexOf(",")+1,top.getValue().length()));
							//	System.out.println("last move : " +sol2.getActions().get(sol2.getActions().size()-1).getName() );
						...
						for (Predicate player : kb.getPredicates()) {
							if (player.getType().startsWith("sokobanAt")) {
								...
								colp = Integer.parseInt(player.getValue().substring(player.getValue().indexOf(',') + 1,	player.getValue().length()));
								sokoban = new Position(rowp, colp);
								break loop;
							}
						}
						destrow =Integer.parseInt(top.getValue().substring(0, top.getValue().indexOf(',')));
						destcol =Integer.parseInt(top.getValue().substring(top.getValue().indexOf(',') + 1,	top.getValue().length()));
