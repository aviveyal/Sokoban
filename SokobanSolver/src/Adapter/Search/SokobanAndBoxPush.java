...
/**
 * 
 * @author Aviv Eyal Adapter - sokoban level to a searchable problem this helps
 *         move player with the box toghter calculate box steps and mini steps
 *         for the player for get the right place arounf the box to push it
 */
...
public class SokobanAndBoxPush implements Searchable<SokobanStateMove> {
	...
	public State<SokobanStateMove> getGoalState() {
		...
		// check last action on path box to target to know where the player will
		// be on goal state
	...
	private List<PositionAndActions> getEmptyCell(char[][] l, Position player, Position box) // player=box
																								// ,
																								// box=
																								// target
	{
	...
	public HashMap<Action, State<SokobanStateMove>> getAllPossibleMoves(State<SokobanStateMove> state) {
		...
		if (pos != null) {
			for (int i = 0; i < pos.size(); i++) {
				...
				if (pos.get(i).getPos().getRow() == boxpos.getRow() + 1
						&& pos.get(i).getPos().getCol() == boxpos.getCol()) {

					Position newbox = new Position(pos.get(i).getPos().getRow(), pos.get(i).getPos().getCol());

					copyboard[pos.get(i).getPos().getRow()][pos.get(i).getPos()
							.getCol()] = copyboard[boxpos.getRow()][boxpos.getCol()];
					...
					// System.out.println("added pos box : "+ newbox.getRow()
					// +","+ newbox.getCol());

				} else if (pos.get(i).getPos().getRow() == boxpos.getRow() - 1
						&& pos.get(i).getPos().getCol() == boxpos.getCol()) {
					Position newbox = new Position(pos.get(i).getPos().getRow(), pos.get(i).getPos().getCol());
				...
				} else if (pos.get(i).getPos().getRow() == boxpos.getRow()
						&& pos.get(i).getPos().getCol() == boxpos.getCol() + 1) {
					Position newbox = new Position(pos.get(i).getPos().getRow(), pos.get(i).getPos().getCol());
					copyboard[pos.get(i).getPos().getRow()][pos.get(i).getPos()
							.getCol()] = copyboard[boxpos.getRow()][boxpos.getCol()];
					copyboard[boxpos.getRow()][boxpos.getCol()] = 'A';
